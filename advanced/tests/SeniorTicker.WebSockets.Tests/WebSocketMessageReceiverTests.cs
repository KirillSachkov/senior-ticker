...
public class WebSocketMessageReceiverTests
{
    ...
    public async Task Reassembles_a_multi_fragment_message_larger_than_initial_buffer()
    {
        ...
        // Паттерн i&0xFF делает payload порядко-зависимым: SequenceEqual проверяет и длину, и ПОРЯДОК байт.
    ...
    [Fact]
    public async Task Throws_when_message_exceeds_non_bucket_max()
    {
        // maxMessageBytes = 5000 — НЕ степень двойки. Rent(5000) отдаёт 8192-байтный массив из пула,
        // поэтому лимит нельзя мерить по buffer.Length: 5001-байтное сообщение обязано упасть (фикс #8).
        var first = new byte[4000];
        var second = new byte[1001]; // суммарно 5001 > 5000
        var socket = new FakeWebSocket([(first, false, false), (second, true, false)]);
        var receiver = new WebSocketMessageReceiver(initialBufferBytes: 1024, maxMessageBytes: 5000);
    ...
    [Fact]
    public async Task Accepts_message_exactly_at_non_bucket_max()
    {
        // Ровно maxMessageBytes (5000) должно ПРОЙТИ, хотя пул отдаёт буфер большего размера.
        var first = new byte[4000];
        var second = new byte[1000]; // суммарно ровно 5000
        var socket = new FakeWebSocket([(first, false, false), (second, true, false)]);
        var receiver = new WebSocketMessageReceiver(initialBufferBytes: 1024, maxMessageBytes: 5000);
        ...
        Assert.False(msg.IsClosed);
        Assert.Equal(5000, msg.Span.Length);
    }

    [Fact]
    public async Task Throws_on_empty_non_final_frame_instead_of_hot_spinning()
    {
        // Пустой не-финальный не-close фрейм (Count=0, EndOfMessage=false) не должен крутить hot-loop —
        // приёмник обязан явно упасть.
        var socket = new FakeWebSocket([(Array.Empty<byte>(), false, false)]);
        var receiver = new WebSocketMessageReceiver(initialBufferBytes: 1024, maxMessageBytes: 2048);
